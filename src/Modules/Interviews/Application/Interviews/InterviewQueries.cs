...
public sealed record InterviewListItemDto(
    Guid Id, Guid ApplicationId, string Type, DateTime ScheduledAtUtc,
...
public sealed record ListInterviewsQuery(
    ...
    int Page = 1, int PageSize = 20) : IQuery<PagedResult<InterviewListItemDto>>;
    ...
    : IQueryHandler<ListInterviewsQuery, PagedResult<InterviewListItemDto>>
{
    ...
    public ListInterviewsHandler(IInterviewsDbContext db) => _db = db;
        ...
        ListInterviewsQuery query, CancellationToken ct)
    {
        ...
        var items = rows
            .Select(i => new InterviewListItemDto(
                i.Id, i.ApplicationId, i.Type.ToString(), i.ScheduledAtUtc,
                i.DurationMinutes, i.Status.ToString(), i.InterviewerUserIds.ToList()))
            .ToList();
